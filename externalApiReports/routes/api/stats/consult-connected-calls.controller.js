...
const getSum = (data) => {
  ...
  sum_data.AGENT_QUEUE_ENTERED = sumData;

  sumData = sum(pluck('AGENT_CONNECTED', data));
  sum_data.AGENT_CONNECTED = sumData;

  sumData = sum(pluck('AGENT_QUEUE_ABANDONED', data));
  sum_data.AGENT_QUEUE_ABANDONED = sumData;

  sumData = sum(pluck('AGENT_RING_ABANDONED', data));
  sum_data.AGENT_RING_ABANDONED = sumData;

  sumData = sum(pluck('SYSTEM_ABANDONED', data));
  sum_data.SYSTEM_ABANDONED = sumData;

  sumData = sum(pluck('FORWARDED', data));
  sum_data.FORWARDED = sumData;
...
const read = async (req, res, next) => {
  try {
    ...
    console.log("consult-connect-calls")
