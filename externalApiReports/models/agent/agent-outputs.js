...
const getAgentOutputs = async ({ date_unit, tenant_key, site_cd, group_key, agent_key, start_date, end_date, start_time, end_time, inbound_type_keys, outbound_type_keys, internal_type_keys }) => {
    ...
    if ( date_unit=="hourly" && isNotEmpty(start_time) ) {
        time_range_query = `  AND RIGHT(DT_KEY, 5) BETWEEN '${start_time}' AND '${end_time}'`;
    }
    ...
    SELECT 
            ...
            , LEFT(DT_KEY,10) AS DATE_KEY, (CASE WHEN LEN(X.DT_KEY) = 16 THEN RIGHT(DT_KEY, 5) ELSE '' END) AS TIME_KEY, AGENT_NAME
            , IB_OFFERED, IB_ENGAGE, IB_ENGAGE_TIME, OB_OFFERED, OB_ENGAGE, OB_ENGAGE_TIME, IN_OFFERED, IN_ENGAGE, IN_ENGAGE_TIME
            , TRANSFER_INIT_AGENT, TRANSFER_ACCEPTED
    ...
    SELECT 
            ...
            , (ISNULL(IB_XFER_RECEIVED_ACCEPTED, 0) + ISNULL(OB_XFER_RECEIVED_ACCEPTED, 0)) AS TRANSFER_ACCEPTED	-- 호전환수신건수
